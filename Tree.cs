...
namespace BinaryTree
{
    ...
    public class Tree
    {
        ...
        public void CountSum(int info)// info - указанный элемент
        {
            ItemCountSum(this.root, info);
        }

        private void ItemCountSum(Item item, int info)
        {
            countSum += NumberDigitSum(item.info);
            if (info < item.info && item.lSon != null)
            {
                ItemCountSum(item.lSon, info);
            }
            if (info > item.info && item.rSon != null)
            {
                ItemCountSum(item.rSon, info);
            }
        }

        private int NumberDigitSum(int number)
        {
            int a = 0;
            while (number > 0)
            {
